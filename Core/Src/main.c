...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
static void encoderUpdate(uint32_t uTicks){
...
int main(void)
{
  ...
  uint32_t uEncoderThreshold = 1000;
  uint32_t uCurrentTicks = 0;
  while (1)
  {
    uCurrentTicks = HAL_GetTick();
    uEncoderTicks += uCurrentTicks;
    if(uEncoderTicks >= uEncoderThreshold){
      uint16_t uVal = __HAL_TIM_GET_COUNTER(&htim1);
      printSerialWrap("Encoder value is %d", uVal);
      
      uEncoderTicks -= uEncoderThreshold;
      HAL_Delay(100);
    }
    /* USER CODE END WHILE */
